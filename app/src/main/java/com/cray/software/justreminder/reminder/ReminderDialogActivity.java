...
public class ReminderDialogActivity extends Activity implements TextToSpeech.OnInitListener, SendListener,
    ...
    private ColorSetter cs = ColorSetter.getInstance(ReminderDialogActivity.this);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (type.contains(Constants.TYPE_CALL) || type.matches(Constants.TYPE_SKYPE) ||
            ...
            if (!type.startsWith(Constants.TYPE_SKYPE)) {
                ...
                long conID = Contacts.getIdFromNumber(number, ReminderDialogActivity.this);
                Uri photo = Contacts.getPhoto(conID);
                if (photo != null) contactPhoto.setImageURI(photo);
                else contactPhoto.setVisibility(View.GONE);
                name = Contacts.getNameFromNumber(number, ReminderDialogActivity.this);
                if (name == null) name = "";
            ...
            if (conID != 0) {
                ...
                name = Contacts.getNameFromMail(number, ReminderDialogActivity.this);
                if (name == null) name = "";
                contactInfo.setText(name + "\n" + number);
            } else {
                contactInfo.setText(number);
        ...
        if (isRepeat) {
            repeater.setAlarm(ReminderDialogActivity.this, id);
        }
    ...
    private void call() {
        Reminder.update(ReminderDialogActivity.this, id);
        String type = getType();
        if (type.contains(Constants.TYPE_MESSAGE)){
            sendSMS(number, task);
        } else if (type.matches(Constants.TYPE_SKYPE)){
            Telephony.skypeCall(number, ReminderDialogActivity.this);
        } else if (type.matches(Constants.TYPE_SKYPE_VIDEO)){
            Telephony.skypeVideoCall(number, ReminderDialogActivity.this);
        } else if (type.matches(Constants.TYPE_SKYPE_CHAT)){
            Telephony.skypeChat(number, ReminderDialogActivity.this);
        } else if (type.contains(Constants.TYPE_APPLICATION)){
            openApplication(number);
        } else if (type.matches(Constants.TYPE_MAIL)){
            Telephony.sendMail(ReminderDialogActivity.this, number, subject, task, attachment);
        } else {
    ...
    private void delay() {
        ...
        Reminder.setDelay(ReminderDialogActivity.this, id, delay, true);
        update(1);
        finish();
    }
    ...
    private void loadData() {
        ...
        TaskListRecyclerAdapter shoppingAdapter = new TaskListRecyclerAdapter(this, provider,
            ...
            public void onItemCheck(int position, boolean isChecked) {
                ...
                Reminder.switchItem(ReminderDialogActivity.this, id, isChecked, provider.getItem(position).getUuId());
                loadData();
            }
            ...
            public void onItemDelete(int position) {
                ...
                Reminder.hideItem(ReminderDialogActivity.this, id, provider.getItem(position).getUuId());
                loadData();
            }
    ...
    private void update(int i){
        ...
        new RepeatNotificationReceiver().cancelAlarm(ReminderDialogActivity.this, id);
        notifier.discardNotification(id);
    }
    ...
    public void openApplication(String number) {
        ...
        notifier.discardNotification(id);
        repeater.cancelAlarm(ReminderDialogActivity.this, id);
        finish();
    }
    ...
    public void showDialog(){
        ...
        builder.setItems(items, (dialog, item1) -> {
            ...
            Reminder.setDelay(ReminderDialogActivity.this, id, x, true);
            Messages.toast(ReminderDialogActivity.this, getString(R.string.reminder_snoozed));
            dialog.dismiss();
            removeFlags();
            finish();
        });
