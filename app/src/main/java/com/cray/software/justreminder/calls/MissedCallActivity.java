...
public class MissedCallActivity extends Activity implements GoogleApiClient.ConnectionCallbacks, DataApi.DataListener {

    private MissedCallAlarmReceiver alarm = new MissedCallAlarmReceiver();
    private long id;
    private ColorSetter cs = ColorSetter.getInstance(MissedCallActivity.this);
    private Notifier notifier = new Notifier(MissedCallActivity.this);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String name = Contacts.getNameFromNumber(number, MissedCallActivity.this);

        wearMessage = name + "\n" + number;

        if (number != null) {
            long conID = Contacts.getIdFromNumber(number, MissedCallActivity.this);
    ...
    private void call() {
        removeMissed();
        if (Permissions.checkPermission(MissedCallActivity.this, Permissions.CALL_PHONE)) {
            Telephony.makeCall(number, MissedCallActivity.this);
            removeFlags();
            finish();
        } else {
            Permissions.requestPermission(MissedCallActivity.this, 104, Permissions.CALL_PHONE);
        }
    }
    ...
    private void removeMissed() {
        ...
        DataBase db = new DataBase(MissedCallActivity.this);
        db.open();
        ...
        switch (requestCode){
            case 104:
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED){
                    ...
                    Permissions.showInfo(MissedCallActivity.this, Permissions.CALL_PHONE);
                }
                break;
        }
    }
