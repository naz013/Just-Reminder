...
public class EventsImportActivity extends AppCompatActivity implements View.OnClickListener,
    ...
    private ArrayList<CalendarHelper.CalendarItem> list;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ColorSetter cs = ColorSetter.getInstance(EventsImportActivity.this);
        ...
        syncInterval.setOnClickListener(v -> Dialogues.selectInterval(EventsImportActivity.this, Prefs.AUTO_CHECK_FOR_EVENTS_INTERVAL, R.string.interval));
    ...
    private void loadCalendars() {
        list = new CalendarHelper(this).getCalendarsList();

        if (list == null || list.size() == 0) {
            Messages.toast(EventsImportActivity.this, getString(R.string.no_calendars_found));
            finish();
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.button:
                if (Permissions.checkPermission(EventsImportActivity.this, Permissions.READ_CALENDAR,
                        Permissions.WRITE_CALENDAR)) {
                    importEvents();
                } else {
                    Permissions.requestPermission(EventsImportActivity.this, 102,
    ...
    private void importEvents() {
        if (!eventsCheck.isChecked()) {
            Messages.toast(EventsImportActivity.this, getString(R.string.no_action_selected));
            return;
        }

        if (eventCalendar.getSelectedItemPosition() == 0) {
            Messages.toast(EventsImportActivity.this, getString(R.string.you_dont_select_any_calendar));
            return;
        }
            ...
            case R.id.autoCheck:
                if (isChecked) {
                    if (Permissions.checkPermission(EventsImportActivity.this, Permissions.READ_CALENDAR,
                        ...
                        Permissions.requestPermission(EventsImportActivity.this, 101,
                                Permissions.READ_CALENDAR, Permissions.WRITE_CALENDAR);
                    }
        ...
        switch (requestCode) {
            case 101:
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Permissions.showInfo(EventsImportActivity.this, Permissions.READ_CALENDAR);
                }
                break;
            case 102:
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
    ...
    public class Import extends AsyncTask<HashMap<String, Integer>, Void, Integer> {
        ...
        protected final Integer doInBackground(HashMap<String, Integer>... params) {
            ...
            CalendarHelper cm = new CalendarHelper(mContext);
            long currTime = System.currentTimeMillis();

            int eventsCount = 0;
            HashMap<String, Integer> map = params[0];
            if (map.containsKey(EVENT_KEY)) {
                ArrayList<CalendarHelper.EventItem> eventItems = cm.getEvents(map.get(EVENT_KEY));
        ...
        protected void onPostExecute(Integer result) {
            ...
            if (result == 0) Messages.toast(EventsImportActivity.this, getString(R.string.no_events_found));

            if (result > 0) {
                Messages.toast(EventsImportActivity.this, result + " " + getString(R.string.events_found));
