...
public class CustomCursorAdapter extends CursorAdapter implements Filterable {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.editButton = (ImageButton) convertView.findViewById(R.id.buttonEdit);
            holder.editButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    long buttonId = 0;
                    if (arrayList != null && arrayList.size() > position)
                        buttonId = arrayList.get(position);

                    if (buttonId > 0) mId = buttonId;
                    if (mId != 0) {
                        Intent intentId = new Intent(mContext, ReminderManager.class);
                        intentId.putExtra(Constants.EDIT_ID, mId);
                        new AlarmReceiver().cancelAlarm(mContext, mId);
                        new WeekDayReceiver().cancelAlarm(mContext, mId);
                        new DelayReceiver().cancelAlarm(mContext, mId);
                        new PositionDelayReceiver().cancelDelay(mContext, mId);
        ...
        ImageView leftTime = (ImageView) convertView.findViewById(R.id.leftTime);
        ...
        if (mDark){
            ...
            holder.editButton.setImageResource(R.drawable.ic_create_white_24dp);
        } else {
            repeatInterval.setBackgroundDrawable(cContext.getResources().getDrawable(R.drawable.round_view_black));
            holder.editButton.setImageResource(R.drawable.ic_create_grey600_24dp);
        }
        ...
        if (isDone == 1){
            leftTime.setImageDrawable(cContext.getResources().getDrawable(R.drawable.drawable_grey));
        }
        return convertView;
    }
