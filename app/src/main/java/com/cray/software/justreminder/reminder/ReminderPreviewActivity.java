...
import com.cray.software.justreminder.notes.NotePreviewActivity;
...
public class ReminderPreviewActivity extends AppCompatActivity implements ActionCallbacks {
    ...
    private void openTask(long id) {
        startActivity(new Intent(this, TaskActivity.class).putExtra(Constants.ITEM_ID_INTENT, id)
    ...
    private void editReminder() {
        if (id != 0) {
            Reminder.edit(id, ReminderPreviewActivity.this);
        }
    }
    ...
    private void initViews() {
        ...
        switchWrapper.setOnClickListener(v -> {
            Reminder.toggle(id, ReminderPreviewActivity.this, ReminderPreviewActivity.this);
            loadInfo();
        });
    ...
    private void findNote() {
        new Thread(new Runnable() {
            NoteItem noteItem = NoteHelper.getInstance(ReminderPreviewActivity.this).getNoteByReminder(id);
            @Override
            public void run() {
                runOnUiThread(() -> {
                    if (noteItem != null) {
    ...
    private void findTask() {
        new Thread(new Runnable() {
            TaskItem taskItem = TasksHelper.getInstance(ReminderPreviewActivity.this).getTaskByReminder(id);
            @Override
            public void run() {
                runOnUiThread(() -> {
                    if (taskItem != null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (ids) {
            case R.id.action_delete:
                Reminder.moveToTrash(id, ReminderPreviewActivity.this, null);
                closeWindow();
                return true;
    ...
    public void showDialog() {
        ...
        builder.setItems(time.toArray(new String[time.size()]), (dialog, which) -> {
            Reminder.copy(id, list.get(which), ReminderPreviewActivity.this, null);
            dialog.dismiss();
        });
    ...
    private void openNote(long id) {
        startActivity(new Intent(this, NotePreviewActivity.class).putExtra(Constants.EDIT_ID, id));
    }
